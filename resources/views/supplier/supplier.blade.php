...
<div class="space-y-5 sm:space-y-8">
    ...
    <div class="bg-[#a93cd4] rounded-2xl sm:rounded-3xl p-4 sm:p-8 text-white relative overflow-hidden shadow-xl shadow-purple-900/10">
        <div class="relative z-10 flex items-center gap-3 sm:gap-6">
            <div class="bg-white/20 p-2.5 sm:p-4 rounded-xl sm:rounded-2xl backdrop-blur-md flex-shrink-0">
                <i class="fas fa-user-friends text-lg sm:text-3xl"></i>
            </div>
            <div>
                <h2 class="text-lg sm:text-2xl font-bold">Supplier</h2>
                <div class="flex items-center gap-2 text-purple-100 text-[9px] sm:text-xs mt-1">
    ...
    <div class="bg-white rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm overflow-hidden p-4 sm:p-6 space-y-4 sm:space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            <div class="flex flex-1 items-center gap-3 w-full sm:max-w-2xl">
                ...
                <form method="GET" action="{{ route('supplier.index') }}" class="flex gap-2 flex-1">
                    <div class="relative flex-1">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Supplier..." class="w-full px-4 py-2 sm:px-5 sm:py-3 bg-[#f1f5f9] border-none rounded-xl text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all placeholder:text-slate-400">
                    </div>
            ...
            <button @click="$dispatch('open-supplier-modal', { mode: 'add', action: '{{ route('supplier.store') }}' })" class="w-full sm:w-auto bg-[#1e40af] hover:bg-blue-800 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-2.5 shadow-lg shadow-blue-200 transition-all">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-[#2d46b9] text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-left">
                        ...
                        <th class="px-3 py-2 sm:px-6 sm:py-4">Nama Supplier</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 text-center">Kontak Person</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4">No. Telepon</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4">Alamat</th>
                        @if(auth()->user()->isAdmin())
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-3 py-3 sm:px-6 sm:py-6 text-xs sm:text-sm font-bold text-slate-800">{{ str_pad($index + 1 + ($suppliers->currentPage() - 1) * $suppliers->perPage(), 2, '0', STR_PAD_LEFT) }}</td>
                        <td class="px-3 py-3 sm:px-6 sm:py-6 text-xs sm:text-sm font-extrabold text-slate-800 uppercase tracking-tight">{!! nl2br(e($supplier->nama)) !!}</td>
                        <td class="px-3 py-3 sm:px-6 sm:py-6 text-xs sm:text-sm text-slate-600 font-medium text-center leading-relaxed">{!! nl2br(e($supplier->kontak_person)) !!}</td>
                        <td class="px-3 py-3 sm:px-6 sm:py-6 text-xs sm:text-sm text-slate-600 font-medium leading-relaxed">{{ $supplier->telepon }}</td>
                        <td class="px-3 py-3 sm:px-6 sm:py-6 text-xs sm:text-sm text-slate-500 font-medium max-w-xs truncate" title="{{ $supplier->alamat }}">{{ $supplier->alamat }}</td>
                        @if(auth()->user()->isAdmin())
                        <td class="px-3 py-3 sm:px-6 sm:py-8">
                            <div class="flex justify-center gap-2.5">
                                ...
                                <button @click="showDeleteModal = true; deleteTarget = '{{ $supplier->nama }}'; deleteAction = '{{ route('supplier.destroy', $supplier->id) }}'" class="p-1 text-slate-400 hover:text-red-600 transition-colors">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-3 py-6 sm:px-6 sm:py-8 text-center text-slate-400 font-medium text-xs sm:text-sm">Tidak ada supplier ditemukan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
