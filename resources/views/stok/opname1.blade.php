...
<div class="space-y-5 sm:space-y-8">
    ...
    <div class="bg-[#d35400] rounded-2xl sm:rounded-3xl p-4 sm:p-8 text-white relative overflow-hidden shadow-xl shadow-orange-900/10">
        <div class="relative z-10 flex items-center gap-4 sm:gap-6">
            ...
            <div>
                <h2 class="text-lg sm:text-2xl font-bold">Stok Opname</h2>
    ...
    <div class="bg-white rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm overflow-hidden p-4 sm:p-6">
        @if(auth()->user()->isAdmin())
        <div class="mb-6 flex items-center justify-between">
            <button @click="showModal = true; modalType = 'add-period'" class="bg-[#2d46b9] hover:bg-blue-800 text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-bold flex items-center gap-2 shadow-lg shadow-blue-200 transition-all">
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-[#2d46b9] text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest">
                        <th class="px-3 py-2 sm:px-6 sm:py-4 rounded-tl-xl text-center whitespace-nowrap">No</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 text-center whitespace-nowrap">Periode</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 text-center whitespace-nowrap">Keterangan</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 text-center whitespace-nowrap">Jumlah Barang</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 text-center whitespace-nowrap">Jumlah Sesuai</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 text-center whitespace-nowrap">Jumlah Selisih</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 text-center whitespace-nowrap">Status Kerja</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 text-center whitespace-nowrap">Status Pelaporan</th>
                        <th class="px-3 py-2 sm:px-6 sm:py-4 rounded-tr-xl text-center whitespace-nowrap">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @forelse($periodes as $index => $row)
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-xs sm:text-sm text-slate-400 font-medium text-center">{{ str_pad($index + 1 + ($periodes->currentPage() - 1) * $periodes->perPage(), 2, '0', STR_PAD_LEFT) }}</td>
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-xs sm:text-sm font-extrabold text-[#2d46b9] leading-relaxed">
                            {{ $row->tanggal_mulai->locale('id')->isoFormat('DD MMM YYYY') }} s/d {{ $row->tanggal_selesai->locale('id')->isoFormat('DD MMM YYYY') }}
                        </td>
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-xs sm:text-sm text-slate-600 font-semibold max-w-xs">
                            <div class="truncate mb-1.5">{{ $row->keterangan }}</div>
                            @if($row->status_pelaporan === 'selesai' || $row->status_pelaporan === 'lengkap')
                                @if($row->is_adjusted)
                                    <span class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-emerald-50 text-emerald-600 text-[8px] sm:text-[9px] font-black uppercase rounded border border-emerald-100" title="Stok Telah Sinkron">
                                        <i class="fas fa-check-circle"></i> Sinkron
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-blue-50 text-blue-600 text-[8px] sm:text-[9px] font-black uppercase rounded border border-blue-100" title="Siap Sinkron - Buka Form Input untuk Menyesuaikan">
                                        <i class="fas fa-exclamation-circle text-blue-500"></i> Siap Sinkron
                                    </span>
                                @endif
                            @endif
                        </td>
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-xs sm:text-sm font-medium text-slate-500 text-center">{{ $row->total_barang }}</td>
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-xs sm:text-sm font-medium text-slate-500 text-center">{{ $row->total_sesuai }}</td>
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-xs sm:text-sm font-medium text-slate-500 text-center">{{ $row->total_selisih }}</td>
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-xs sm:text-sm font-semibold text-center">
                            @if($row->status_kerja === 'aktif')
                                <span class="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-emerald-50 text-emerald-600 text-[8px] sm:text-[9px] font-black uppercase rounded">{{ $row->status_kerja }}</span>
                            @else
                                <span class="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-slate-50 text-slate-400 text-[8px] sm:text-[9px] font-black uppercase rounded">{{ $row->status_kerja }}</span>
                            @endif
                        </td>
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-center">
                            @if($row->status_pelaporan === 'lengkap' || $row->status_pelaporan === 'selesai')
                                <span class="px-2 sm:px-3 py-0.5 sm:py-1 bg-emerald-50 text-emerald-600 text-[8px] sm:text-[10px] font-black uppercase rounded-full">{{ str_replace('_', ' ', $row->status_pelaporan) }}</span>
                            @else
                                <span class="px-2 sm:px-3 py-0.5 sm:py-1 bg-rose-50 text-rose-600 text-[8px] sm:text-[10px] font-black uppercase rounded-full">{{ str_replace('_', ' ', $row->status_pelaporan) }}</span>
                            @endif
                        </td>
                        <td class="px-3 py-3 sm:px-6 sm:py-5 text-center">
                            <div class="flex items-center justify-center gap-1 sm:gap-2">
                                <a href="{{ route('stok.opname2', ['periode_id' => $row->id]) }}" class="p-1 sm:p-2 text-slate-400 hover:text-blue-600 transition-colors" title="Input Stok Fisik">
                                    <i class="fas fa-edit text-xs sm:text-sm"></i>
                                </a>
                                <a href="{{ route('stok.opname3', ['periode_id' => $row->id]) }}" class="p-1 sm:p-2 text-slate-400 hover:text-emerald-600 transition-colors" title="Lihat Laporan">
                                    <i class="fas fa-chart-line text-xs sm:text-sm"></i>
                                </a>
                                 @if(auth()->user()->isAdmin())
                                 <button type="button" @click="$dispatch('open-edit-modal', { action: '{{ route('stok.updatePeriode', $row->id) }}', keterangan: '{{ addslashes($row->keterangan) }}' })" class="p-1 sm:p-2 text-slate-400 hover:text-blue-600 transition-colors" title="Edit Keterangan">
                                     <i class="fas fa-pen text-[10px] sm:text-xs"></i>
                                 </button>
                                <button type="button" @click="showDeleteModal = true; deleteTarget = 'Periode Opname {{ $row->tanggal_mulai->locale('id')->isoFormat('DD MMM YYYY') }} s/d {{ $row->tanggal_selesai->locale('id')->isoFormat('DD MMM YYYY') }}'; deleteAction = '{{ route('stok.destroyPeriode', $row->id) }}'" class="p-1 sm:p-2 text-slate-400 hover:text-rose-600 transition-colors" title="Hapus Periode">
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="px-3 py-6 sm:px-6 sm:py-8 text-center text-slate-400 font-medium text-xs sm:text-sm">Belum ada periode stok opname.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
            <p class="text-[11px] sm:text-xs text-slate-400 font-medium tracking-tight">
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    <a href="{{ $periodes->previousPageUrl() }}" class="px-3 py-1.5 sm:px-4 sm:py-2 border border-slate-200 rounded-xl text-[11px] sm:text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all">Sebelumnya</a>
                @endif

                @if ($periodes->hasMorePages())
                    <a href="{{ $periodes->nextPageUrl() }}" class="px-3 py-1.5 sm:px-4 sm:py-2 border border-slate-200 rounded-xl text-[11px] sm:text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all">Selanjutnya</a>
                @else
