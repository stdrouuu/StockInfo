...
@section('title', 'StockInfo - Pencocokan Stok Fisik (Opname)')
        ...
        <div class="relative z-10 flex items-center justify-between gap-6">
            <div>
                ...
                <div class="flex items-center gap-2 text-orange-100 text-[10px] mt-1 font-bold">
                    ...
                    <i class="fas fa-chevron-right text-[8px]"></i>
                    <span class="text-white uppercase font-black tracking-widest">INPUT COCOKKAN STOK</span>
                </div>
            </div>
            <div class="bg-white/10 px-5 py-3 rounded-2xl backdrop-blur-md text-right">
                <span class="text-xs text-orange-100 font-bold block">Jadwal Pengecekan</span>
                <span class="text-sm font-black">{{ $periode->tanggal_mulai->format('d M Y') }} s/d {{ $periode->tanggal_selesai->format('d M Y') }}</span>
            </div>
        </div>
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="bg-[#1e40af] text-white text-[10px] font-black uppercase tracking-widest">
                    ...
                    <th class="px-6 py-4 text-center">Stok di Komputer (Sistem)</th>
                    <th class="px-6 py-4 text-center">Stok Nyata di Toko (Fisik)</th>
                    <th class="px-6 py-4 text-center">Selisih Stok</th>
                    <th class="px-6 py-4 text-center">Status Kecocokan</th>
                    <th class="px-6 py-4 text-center">Alasan Selisih</th>
                    <th class="px-6 py-4 rounded-tr-2xl text-center">Laporkan</th>
                </tr>
            </thead>
                ...
                <tr class="hover:bg-slate-50/50">
                    ...
                    <td class="px-6 py-6 text-center">
                        ...
                        @elseif($item->selisih === 0)
                            <span class="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black rounded-full uppercase">SESUAI (AMAN)</span>
                        @else
                            <span class="px-3 py-1 bg-rose-50 text-rose-600 text-[10px] font-black rounded-full uppercase font-black">SELISIH (BEDA)</span>
                        @endif
                    </td>
                    <td class="px-6 py-6 text-sm text-slate-400 text-center italic">{{ $item->catatan === 'belum dilaporkan' ? 'Belum dicatat' : $item->catatan }}</td>
                    <td class="px-6 py-6 text-center">
                        ...
                        })" class="bg-[#1e40af] text-white text-[10px] font-black px-4 py-1.5 rounded-lg uppercase shadow-lg shadow-blue-100 hover:bg-blue-800 transition-colors">Cocokkan Stok</button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="9" class="px-6 py-8 text-center text-slate-400 font-medium">Tidak ada barang dalam jadwal pengecekan stok ini.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div x-show="modalType === 'report-opname'"
     ...
     ">
    <h2 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.25em] mb-8">Pencocokan Stok Fisik Toko</h2>
    <form :action="action" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-black text-slate-800 uppercase tracking-widest block ml-1">Kode Barang</label>
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-black text-slate-800 uppercase tracking-widest block ml-1">Jumlah Stok di Komputer (Sistem)</label>
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-black text-slate-800 uppercase tracking-widest block ml-1">Jumlah Nyata Dihitung di Rak (Stok Fisik)</label>
        ...
        <div class="space-y-2">
            <label class="text-[10px] font-black text-slate-800 uppercase tracking-widest block ml-1">Alasan Selisih Stok (Contoh: Semen bocor, Besi karat...)</label>
            <textarea name="catatan" x-model="catatan" rows="4" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm text-slate-700 font-medium focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition-all resize-none" placeholder="Contoh: Ada 2 sak semen pecah/bocor saat ditumpuk..."></textarea>
        </div>
        <div class="flex items-center justify-end gap-3 pt-4">
            ...
            <button type="submit" class="px-10 py-3.5 bg-[#2d46b9] hover:bg-blue-800 text-white text-sm font-bold rounded-xl shadow-lg shadow-blue-200 transition-all">Simpan Hasil Cocok Stok</button>
        </div>
    </form>
</div>
@endsection
