...
<div class="max-w-4xl">
    ...
    <div class="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div class="flex items-center gap-8">
            <div class="relative">
                <div class="w-24 h-24 bg-[#1e40af] rounded-[2rem] flex items-center justify-center text-white text-3xl font-bold shadow-lg shadow-blue-100">
                    {{ substr(session('user.name', 'Administrator'), 0, 2) }}
                </div>
                <button class="absolute -bottom-1 -right-1 bg-white p-2 rounded-xl shadow-md border border-slate-100 text-blue-600">
                ...
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <span class="text-sm text-slate-400 font-medium">Admin Level 4</span>
        ...
        <div class="mt-12 pt-10 border-t border-slate-50">
            <div class="max-w-sm">
                ...
                <p class="text-lg font-bold text-slate-700 border-b border-slate-100 pb-2">{{ session('user.name', 'Administrator') }}</p>
            </div>
            <div class="max-w-sm mt-6">
                <label class="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.2em] mb-3 block">Username</label>
                <p class="text-lg font-bold text-slate-700 border-b border-slate-100 pb-2">{{ session('user.username', 'admin') }}</p>
            </div>
        </div>
    </div>
